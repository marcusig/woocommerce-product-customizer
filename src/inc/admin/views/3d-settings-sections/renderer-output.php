...
<p class="field-row">
	<label for="pc-3d-tone-mapping"><?php _e( 'Tone mapping', 'product-configurator-for-woocommerce' ); ?></label>
	<select id="pc-3d-tone-mapping" class="pc-3d-tone-mapping pc-3d-option-help-source" data-key="renderer.tone_mapping">
		<option value="aces" data-help="<?php esc_attr_e( 'Rolls bright areas off smoothly instead of clipping them, the way a camera does. Lets you raise exposure to open up dark areas without blowing out highlights.', 'product-configurator-for-woocommerce' ); ?>" <# if ( ! data.renderer || ! data.renderer.tone_mapping || data.renderer.tone_mapping === 'aces' ) { #>selected<# } #>><?php _e( 'Filmic (ACES) — recommended', 'product-configurator-for-woocommerce' ); ?></option>
		<option value="linear" data-help="<?php esc_attr_e( 'Brightness is multiplied and then clipped. Bright areas turn flat white with no detail.', 'product-configurator-for-woocommerce' ); ?>" <# if ( data.renderer && data.renderer.tone_mapping === 'linear' ) { #>selected<# } #>><?php _e( 'Linear', 'product-configurator-for-woocommerce' ); ?></option>
		<option value="none" data-help="<?php esc_attr_e( 'No tone mapping. Values pass through untouched.', 'product-configurator-for-woocommerce' ); ?>" <# if ( data.renderer && data.renderer.tone_mapping === 'none' ) { #>selected<# } #>><?php _e( 'None', 'product-configurator-for-woocommerce' ); ?></option>
	</select>
</p>
