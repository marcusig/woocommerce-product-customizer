		...
		public function load_scripts() {
			...
			if ( $this->_current_screen_is( 'product' ) ) {
				...
				foreach($scripts as $script) {
					...
					if ( $key === 'backbone/views/3d-settings' && ! empty( $admin_3d_loader_deps ) ) {
						$deps = array_merge( $deps, $admin_3d_loader_deps );
					}
					wp_enqueue_script( 'mkl_pc/js/admin/' . $key, MKL_PC_ASSETS_URL . 'admin/js/'. $file , $deps, filemtime( MKL_PC_ASSETS_PATH . 'admin/js/'. $file ), true );
				}
				...
				$pc_lang = array(
					...
					'3d_objects_new_placeholder' => __( '3D object label…', 'product-configurator-for-woocommerce' ),
					...
					'reset_settings_3d_confirm' => __( 'This will restore all 3D viewer settings to their defaults. Continue?', 'product-configurator-for-woocommerce' ),
