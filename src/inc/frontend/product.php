...
if ( ! class_exists('MKL\PC\Frontend_Product') ) {
	...
	class Frontend_Product {
		...
		public function __construct() {
			...
			$this->_hooks();
			$this->button_class = isset( $this->options['mkl_pc__button_classes'] ) && ! empty( $this->options['mkl_pc__button_classes'] ) ? Utils::sanitize_html_classes( $this->options['mkl_pc__button_classes'] ) : 'primary button btn btn-primary wp-element-button';
		}
		...
		public function add_configure_button() { 
			...
			if ( mkl_pc_is_configurable( get_the_id() ) ) {
				...
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Filtered HTML; default markup is escaped, filters may add trusted HTML.
				echo apply_filters(
					'mkl_pc_configure_button_a11y_description',
					'<h2 class="screen-reader-text">' . esc_html_x( 'Press the Configure button to enter the product configurator (next element)', 'Accessibility description for the configure button', 'product-configurator-for-woocommerce' ) . '</h2>',
					$product
				);
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Filtered HTML; dynamic parts are escaped and data-attrs are generated by an internal helper.
				echo apply_filters(
					'mkl_pc_configure_button',
					'<button class="configure-product configure-product-' . $product_type . ' ' . esc_attr( $this->button_class ) . '" ' . $attributes . ' type="button"><span class="configure-button--label">' . esc_html( $label ) . '</span><span class="loading-icon" aria-hidden="true"><i></i></span></button>'
				);
			}
		}
		...
		public function get_product_price( $product_id, $price_type = 'price' ) {
			...
			$process_woocs = false;
			if ( $WOOCS && ! isset( $_REQUEST['woocs_block_price_hook'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Compatibility flag for the WOOCS currency plugin, not form processing.
				$process_woocs = true;
			...
			return apply_filters( 'mkl_pc/get_product_price', $price, $product );
		}
			...
			echo '<# if ( data.is_in_stock ) { #>';
				...
				if ( $product && is_a( $product, 'WC_Product' ) && ! $product->is_sold_individually() ) {
					$qty_input = woocommerce_quantity_input( array(
						...
						// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Quantity is prefilled from the product form; WooCommerce verifies the add-to-cart nonce.
					...
					echo $qty_input; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- WooCommerce quantity input HTML is escaped internally.
				}
				echo '<# } #>';
				?>
					<# if ( data.show_form ) { #>
		...
		public function add_edit_configuration_from_cart() {
			if ( ! isset( $_REQUEST[ 'edit_config_from_cart' ] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Query flag to show the edit-in-cart button.
				return;
			}
			?>
			<button type="button" class="<?php echo esc_attr( $this->button_class ) ?> edit-cart-item configurator-add-to-cart">
				<span><?php
					 ...
					 */
					$label = apply_filters( 'mkl_pc/edit_item_in_cart_button/label', mkl_pc( 'settings' )->get_label( 'edit_item_in_cart', esc_html_x( 'Edit item in cart', 'Label for the edit item in cart button', 'product-configurator-for-woocommerce' ) ) );
					echo esc_html( $label );
				?></span>
			</button>
			<?php 
