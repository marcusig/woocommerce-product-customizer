...
class Rest_Condition_Controller extends Rest_Base_Controller {
	/**
	 * Endpoint namespace.
	 *
	 * @var string
	 ...
	 * @var string
	 */
	protected $rest_base = 'configuration/(?P<product_id>[\d]+)/conditions';
	...
	protected function prepare_object_for_database( $request, $creating = false ) {
		...
		$props = [];
		foreach( $available_props as $setting ) {
	...
	protected function get_objects( $request ) {
		...
		$data = $wpdb->get_col( $wpdb->prepare( "SELECT condition_id FROM {$wpdb->prefix}mklpc_conditions WHERE product_id = %d;", $product_id ) );
