...
if ( ! class_exists( '\MKL\PC\Base_Email', false ) ) :
	 ...
	 *
	 * @class       MK_PC_SYD_Share_Base_Email
	 * @version     1.2
	 * @extends     WC_Email
	 */
	abstract class Base_Email extends \WC_Email {
		...
		public function trigger( $configuration, $form_data ) {
			...
			$this->placeholders['{configuration-table}'] = $this->get_table();
			// $this->link =      $url;
			// $this->placeholders['{link}'] = $url;

			if ( $this->customer_email ) $this->recipient = $form_data['email'];
