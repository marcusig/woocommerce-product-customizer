<?php
function mkl_pc_fiftyfifty_theme_scripts() {
	wp_enqueue_script( 'mkl/pc/themes/fiftyfifty', plugin_dir_url( __FILE__ ) . 'fiftyfifty.js', [ 'wp-hooks', 'jquery', 'mkl_pc/js/vendor/tippy' ], filemtime( plugin_dir_path( __FILE__ ) . 'fiftyfifty.js' ), true );
}
add_action( 'mkl_pc_scripts_product_page_after', 'mkl_pc_fiftyfifty_theme_scripts', 20 );

// Wrap the choice Name and description with a span
function mkl_pc_fiftyfifty_theme_choice_wrapper_open() {
	echo '<span class="choice-text">';
}
add_action( 'tmpl-pc-configurator-choice-item', 'mkl_pc_fiftyfifty_theme_choice_wrapper_open', 6 );

function mkl_pc_fiftyfifty_theme_choice_wrapper_close() {
	echo '</span>';
}
add_action( 'tmpl-pc-configurator-choice-item', 'mkl_pc_fiftyfifty_theme_choice_wrapper_close', 160 );
// END: Wrap the choice Name and description with a span

// // Wrap the choice Name and description with a span
// function mkl_pc_fiftyfifty_theme_layer_wrapper_open() {
// 	echo '<span class="layer-text">';
// }
// add_action( 'tmpl-mkl-pc-configurator-layer-item-button', 'mkl_pc_fiftyfifty_theme_layer_wrapper_open', 6 );

// function mkl_pc_fiftyfifty_theme_layer_wrapper_close() {
// 	echo '</span>';
// }
// add_action( 'tmpl-mkl-pc-configurator-layer-item-button', 'mkl_pc_fiftyfifty_theme_layer_wrapper_close', 160 );
// // END: Wrap the choice Name and description with a span

// function mkl_pc_fiftyfifty_theme_remove_title() {
...
// }
// add_action( 'mkl_pc_frontend_templates_before', 'mkl_pc_fiftyfifty_theme_remove_title', 20 );
 ...
 * @return void
 */
function mkl_pc_fiftyfifty_theme_move_extra_price() {
...
add_action( 'mkl_pc_frontend_configurator_footer_section_left_before', 'mkl_pc_fiftyfifty_theme_move_extra_price', 40 );

// Move thumbnail
add_action( 'tmpl-pc-configurator-choice-item', 'mkl_pc_fiftyfifty_choice_thumbnail', 1 );

function mkl_pc_fiftyfifty_choice_thumbnail( ) {
...
// function mkl_pc_fiftyfifty_theme_filter_colors( $colors ) {
...
// 	return $colors;
// }
// add_filter( 'mkl_pc_theme_color_settings', 'mkl_pc_fiftyfifty_theme_filter_colors' );

function mkl_pc_fiftyfifty_syd_icon() {
...
add_filter( 'PC.syd.svg.icon', 'mkl_pc_fiftyfifty_syd_icon' );

function mkl_pc_fiftyfifty_add_reset_icon() {
	echo file_get_contents( trailingslashit( MKL_PC_INCLUDE_PATH ) . 'themes-common/icons/reset.svg' );
}
add_action( 'mkl_pc/reset_button/before_label', 'mkl_pc_fiftyfifty_add_reset_icon' );

function mkl_pc_fiftyfifty_save_pdf_icon() {
	echo file_get_contents( trailingslashit( MKL_PC_INCLUDE_PATH ) . 'themes-common/icons/download.svg' );
}
add_action( 'PC.syd.pdf_download.svg.icon', 'mkl_pc_fiftyfifty_save_pdf_icon' );
