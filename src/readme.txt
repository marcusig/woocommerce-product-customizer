...
* FIX: On multilingual sites, the saved layer and choice names are only used in the language the customer configured in. The admin now sees an order placed in another language with the product's own names, instead of the customer's translation: the order's Configuration is rebuilt in the admin's language, while the customer's order pages and emails keep the one saved at checkout
...
* FIX: Compatibility with latest WooCommerce Paypal Payments (caused Add to cart error notice out of context)
...
* FIX: compatibility with YITH RAQ Premium, which was broken by the recently added Free template override
