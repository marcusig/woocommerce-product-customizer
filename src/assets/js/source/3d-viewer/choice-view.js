 ...
 * (material variant, color, texture, placement) for that choice only.
...
const viewer_3d_choice = Backbone.View.extend({
	...
	/**
	 * What actions that move objects (added by add-ons) need to file their
	 * requests: the viewer's placement manager, the viewer itself for any
	 * per-viewer state, a key unique to this choice, and a priority that puts
	 * choices above model positions and later layers above earlier ones.
	 *
	 * @returns {Object}
	 */
	_placement_context() {
