...
class DB { 
	...
	*/
	...
	public function get( $that, $post_id ) {
		...
		$data = maybe_unserialize( get_post_meta( $post_id, '_mkl_product_customizer_' . $that, true ) );
	...
	public function set( $that ) {
		...
		$t = update_post_meta( $id, '_mkl_product_customizer_' . $that , $data ); 
		return 1;
		...
		if( isset($_REQUEST['fe']) && $_REQUEST['fe'] == 1 ) {
			if( $this->is_product( $id ) ) { 
				...
				$init_data['product_info'] = array(
					'title' => apply_filters( 'the_title', $product->post->post_title, $id ),
					'bg_image' => 'http://unoiseaudepapier.loc/wp-content/uploads/2016/05/bg.jpg',
